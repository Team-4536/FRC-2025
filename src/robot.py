...
class Robot(wpilib.TimedRobot):
    def robotInit(self) -> None:
        self.turretMotor = SparkMax(2, rev.SparkMax.MotorType.kBrushless)
        self.photonPose2d = Pose2d(0,0,0)
        # AUTO_SIDE_RED = "red"
        # AUTO_SIDE_BLUE = "blue"
    ...
    def teleopPeriodic(self) -> None:
        ...
        self.turretMotor.setVoltage(0)
        self.tagX = 0
        self.tagY = 0
        self.adjacent = 1
        self.oposite = 1
        if self.photonCamera1.ambiguity < 0.05:
            self.seenTag = self.photonCamera1.fiducialId
            self.tagX = self.aprilTagX[self.seenTag]/39.37
            self.tagY = self.aprilTagY[self.seenTag]/39.37
        #self.table.putNumber("tagx",self.tagX)
        #self.table.putNumber("tagy", self.tagY)
        self.robotX = self.photonPose2d.X()
        self.robotY = self.photonPose2d.Y()
        self.oposite = int (self.tagY) - int(self.robotY)
        self.adjacent = int(self.tagX) - int(self.robotX)
        self.angle = math.atan2(self.oposite,self.adjacent)
        if(abs(self.angle - self.photonPose2d.rotation().radians()) > 0.1):
            if(self.photonCamera1.fiducialId == 17):
                if(self.angle < self.photonPose2d.rotation().radians()):
                    self.turretMotor.setVoltage(0.05)
                if(self.angle > self.photonPose2d.rotation().radians()):
                    self.turretMotor.setVoltage(-0.05)
        ...
        #     else:
        #         self.swerveDrive.savePos(
        #             0, self.swerveDrive.odometry.getPose().rotation().radians()hehe
        #         )
